...
class MedicalQAModel:
    ...
    def __init__(self, model_name="bert-base-uncased", peft_technique="adapter"):
        ...
        torch.nn.init.zeros_(self.model.qa_outputs.bias)
...
class MedicalQAModel:
    """Classe principale du modèle QA médical avec adapter layers"""
    ...
    def __init__(self, model_name="bert-base-uncased", peft_technique="adapter"):
        ...
        Args:
            model_name: Nom du modèle pré-entraîné
            peft_technique: Technique de fine-tuning (adapter/prompt/lora)
        """
        print(f"\nInitializing MedicalQAModel with:")
        print(f"- Model name: {model_name}")
        print(f"- PEFT technique: {peft_technique}")
    ...
    def setup_peft(self):
        """Configure la technique PEFT choisie avec adapter layers"""
        print(f"\nSetting up PEFT with technique: {self.peft_technique}")
        
        if self.peft_technique == "adapter":
            # Configuration des adapter layers
            print("Configuring adapter layers...")
            adapter_config = AdapterConfig(
                hidden_size=self.model.config.hidden_size,
                adapter_size=64,  # Taille des adapter layers (réduite par rapport à la taille cachée originale)
                adapter_act="relu",  # Fonction d'activation
                adapter_initializer_range=0.01,  # Range d'initialisation
                adapter_layers_enum=[0, 1, 2, 3, 4, 5],  # Couches où ajouter les adapters
                adapter_residual=True,  # Utilisation de connexions résiduelles
                ln_after=True,  # Layer normalization après l'adapter
                ln_before=False,  # Pas de layer normalization avant l'adapter
                mh_adapter=True,  # Adapter dans l'attention multi-tête
                output_adapter=True,  # Adapter en sortie
                non_linearity="relu",  # Non-linéarité utilisée
                reduction_factor=16,  # Facteur de réduction pour l'efficacité
            )
            
            # Application des adapter layers
            for layer_idx in adapter_config.adapter_layers_enum:
                if hasattr(self.model.base_model, f'layer_{layer_idx}'):
                    layer = getattr(self.model.base_model, f'layer_{layer_idx}')
                    
                    # Ajout de l'adapter à l'attention
                    if adapter_config.mh_adapter and hasattr(layer, 'attention'):
                        layer.attention.adapter = AdapterLayer(adapter_config)
                    
                    # Ajout de l'adapter à la sortie
                    if adapter_config.output_adapter and hasattr(layer, 'output'):
                        layer.output.adapter = AdapterLayer(adapter_config)
