...
def test_example(model, tokenizer, question, context):
    ...
    with torch.no_grad():
        ...
        # On simule une réponse de référence basée sur une partie du contexte
        # Dans un cas réel, cela devrait venir d'un dataset de test avec des réponses de référence
        true_answer = context[50:100]  # On prend arbitrairement une portion du contexte comme réponse de référence

        # Calcul des différentes métriques
        exact_match = QAMetrics.calculate_exact_match(predicted_answer, true_answer)
        f1_score = QAMetrics.calculate_f1(predicted_answer, true_answer)
        cosine_sim = QAMetrics.calculate_cosine_similarity(predicted_answer, true_answer)

    print("\nRésultats:")
    print(f"Réponse prédite: {predicted_answer}")
    print(f"Réponse de référence: {true_answer}")
    print("\nMétriques:")
