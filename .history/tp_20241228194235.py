...
class QAMetrics:
    ...
    # Nettoyer et comparer les textes
      pred_text = str(pred_text).strip()
      true_text = str(true_text).strip()
    
      return int(pred_text == true_text)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for batch in eval_dataloader:
                ...
                predictions = [batch['input_ids'][i][start_preds[i]:end_preds[i]+1].tolist() for i in range(len(batch))]
                total_predictions.extend(predictions)
                total_labels.extend(batch['end_positions'].tolist())
...
def test_model(model, tokenizer, test_questions):
    ...
    return results
Troubleshooting BERT Model Saving Issue - Claude
...
def main():
    ...
    # Séparation du dataset en train et validation
    small_dataset = dataset["train"].select(range(1000))
    train_val_split = small_dataset.train_test_split(test_size=0.1, seed=42)
    train_data = train_val_split["train"]
    val_data = train_val_split["test"]  # Ce sera notre validation set
